...
import { type ElementType, forwardRef, type HTMLAttributes, type RefObject, useRef } from "react";
import {
  DayPicker,
  ...
  ({ onClick, icon, disabled, ...props }: NavigationButtonProps, forwardedRef) => {
    ...
    return (
      <button
        ...
        disabled={disabled}
        className={cx(
          "flex size-8 shrink-0 select-none items-center justify-center rounded-sm border p-1 outline-hidden transition sm:size-7.5",
          // text color
          ...
          return (
            ...
            >
              ...
              {today && (
                <span
                  className={cx(
                    "absolute inset-x-1/2 bottom-1.5 h-0.5 w-4 -translate-x-1/2 rounded-xs",
                    {
